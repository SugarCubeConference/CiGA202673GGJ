...
namespace GladeAgenticAI.Core.Tools.Implementations.Physics
{
    public class CreatePhysicsMaterialTool : ITool
    {
        ...
        public string Execute(Dictionary<string, object> args)
        {
            ...
            var existing = AssetDatabase.LoadAssetAtPath<PhysicMaterial>(materialPath);
            if (existing != null)
            {
            ...
            var mat = new PhysicMaterial(System.IO.Path.GetFileNameWithoutExtension(materialPath));
            ...
            {
                if (System.Enum.TryParse(args["frictionCombine"].ToString(), true, out PhysicMaterialCombine combine))
                    mat.frictionCombine = combine;
            }
            if (args.ContainsKey("bounceCombine"))
            {
                if (System.Enum.TryParse(args["bounceCombine"].ToString(), true, out PhysicMaterialCombine combine))
